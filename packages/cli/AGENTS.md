...
- **CLI factory**: `createCliProgram()` (`src/cli.ts`) builds the `Command` instance, configures name/description/version (`getCliVersion()`), and registers `check`, `create`, `config`, `dev`, `env`, `start`, `stop`, and `migrate` commands.
...
| `dev [--docker] [--services <list>] [-d] [--env <file>] [--no-env-bootstrap] [--pm <manager>]` | Run the project in development mode using either the package script or Docker compose stack. | When not using Docker it runs the configured dev script (`dev` default) after optional env bootstrapping. Docker mode bootstraps the docker env file, resolves compose overlays, and runs `docker compose --env-file .env.docker ... up --build [-d]` when the Docker env exists. | `src/commands/dev.ts` auto-detects the package manager, persists it into the config, handles env bootstrapping (local or docker), then either calls `runPackageScript()` or `runDockerDev()` (which uses `generateComposeFiles()` with mode `dev`). |
| `start [--docker] [--services <list>] [-d] [--build] [--env <file>] [--env-bootstrap] [--pm <manager>]` | Production entry point mirroring `dev` but defaulting to the configured start script and prod compose overlays. | Local mode optionally bootstraps env files when `--env-bootstrap` is passed; Docker mode layers prod compose files, never pulls Hexabot application images, supports local builds with `--build`, passes `.env.docker` to Compose when present, and logs which services start. | `src/commands/start.ts` shares the package-manager persistence logic with `dev`. `runDockerStart()` enforces Docker availability, resolves compose overlays with mode `prod`, and shells out via `dockerCompose()`. |
| `stop --docker [--services <list>] [-v] [--remove-orphans]` | Tear down the Docker stack started by `dev --docker` / `start --docker`. | `--docker` is mandatory: without it the command errors out (local `dev`/`start` are foreground processes stopped with Ctrl+C). `--volumes` and `--remove-orphans` map to the matching `docker compose down` flags. | `src/commands/stop.ts` resolves services from `--services` or `docker.defaultServices`, builds the `-f` chain with `generateComposeFiles()` **without** a mode overlay, and runs `docker compose --env-file .env.docker ... down` via `dockerCompose()` so the compose project name matches the started stack. |
...
8. `hexabot stop --docker` — tear down the Docker stack (add `-v` to drop volumes).
9. `hexabot migrate` / `hexabot env list` as needed for lifecycle management.
