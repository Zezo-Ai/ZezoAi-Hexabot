...
- **CLI factory**: `createCliProgram()` (`src/cli.ts`) builds the `Command` instance, configures name/description/version (`getCliVersion()`), and registers `check`, `create`, `config`, `dev`, `env`, `start`, and `migrate` commands.
...
8. `hexabot migrate` / `hexabot env list` as needed for lifecycle management.
